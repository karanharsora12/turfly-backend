...
export class ParticipantsService {
  async joinMeetup(userId: string, meetupId: string) {
    ...
    const existingParticipant = meetup.participants.find(p => p.userId === userId);
    if (existingParticipant) {
      const error: any = new Error('You have already joined or requested to join this meetup');
      error.statusCode = 400;
      throw error;
    }
  ...
  async updateParticipantStatus(meetupId: string, targetUserId: string, hostId: string, status: ParticipantStatus) {
    ...
    return prisma.$transaction(async (tx) => {
      ...
      } else if (participant.status === 'APPROVED' && (status === 'REJECTED' || status === 'KICKED')) {
        await tx.meetup.update({
          where: { id: meetupId },
          data: { currentPlayers: { decrement: 1 } }
        });
